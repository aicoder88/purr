...
export default async function handler(
  ...
  try {
    ...
    try {
      ...
      try {
        const jsonUrl =
          "https://script.google.com/macros/s/AKfycbyhSXCHntxmO0fb1IImlzs80doIllEWuHpF-eB72p11SgoN5_xUQf2SXU6Cx7h4XtjabA/exec";
        ...
        if (jsonResponse.ok) {
          console.log(
            "Successfully submitted to Google Apps Script via JSON POST",
          );
          // Return success response instead of exiting early
          return res.status(200).json({
            success: true,
            message:
              "Your free bag request has been submitted successfully! We'll be in touch soon with shipping details.",
          });
        } else {
          console.log(
            `JSON POST method returned status: ${jsonResponse.status}. Trying URL params method...`,
          );
        }
      } catch (jsonError) {
        console.log(
          "Error with JSON POST method, trying URL params method...",
          jsonError,
        );
      }
      ...
      if (!getResponse.ok) {
        console.error(
          `Failed to submit to Google Apps Script: ${getResponse.status} ${getResponse.statusText}`,
        );
        throw new Error(
          `Failed to submit to Google Apps Script: ${getResponse.statusText}`,
        );
      }

      console.log(
        "Successfully submitted to Google Apps Script via URL parameters",
      );
    } catch (error) {
    ...
    console.error(
      "Error processing free giveaway form:",
