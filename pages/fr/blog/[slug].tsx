...
export async function getStaticProps({ params }: { params: { slug: string } }) {
  try {
    ...
    if (!process.env.WORDPRESS_API_URL || process.env.WORDPRESS_API_URL === 'https://your-wordpress-site.com/wp-json/wp/v2') {
      ...
      // Update the link to include the French locale
      post.link = `/fr/blog/${params.slug}`;
...
export default function BlogPost({ post }: { post: BlogPost }) {
  ...
  return (
    <>
      <Head>
        <title>{post.title} | {t.siteName} Blog</title>
        ...
        <meta property="og:image" content={post.image.startsWith('http') ? post.image : `https://purrify.ca${post.image}`} />
        ...
        <meta property="twitter:image" content={post.image.startsWith('http') ? post.image : `https://purrify.ca${post.image}`} />
        
        {/* Canonical Link */}
        <link rel="canonical" href={`https://purrify.ca${post.link}`} />
        
        {/* Language alternates */}
        <link rel="alternate" hrefLang="en" href={`https://purrify.ca/blog/${router.query.slug}`} />
        <link rel="alternate" hrefLang="fr" href={`https://purrify.ca/fr/blog/${router.query.slug}`} />
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              '@type': 'BlogPosting',
              'headline': post.title,
              'image': [post.image.startsWith('http') ? post.image : `https://purrify.ca${post.image}`],
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="mb-8">
                ...
                <div className="flex items-center text-sm text-gray-600 mb-6">
                  <span className="mr-4">{new Date(post.date).toLocaleDateString('fr-CA', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
              ...
              <div className="mt-12 pt-8 border-t border-[#E0EFC7]">
                <h3 className="text-xl font-bold mb-4 text-[#5B2EFF]">Partagez cet article</h3>
                <div className="flex space-x-4">
                  <a
                    href={`https://twitter.com/intent/tweet?text=${post.title}&url=https://purrify.ca${post.link}`}
                    target="_blank"
