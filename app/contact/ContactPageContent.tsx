...
import { CONTACT_INFO, PHONE_MESSAGING, PHONE_NUMBER, SOCIAL_LINKS } from '@/lib/constants';
...
export function ContactPageContent() {
  ...
  const contactMethods = [
    {
      iconName: 'Mail',
      title: contactPage?.contactMethods?.[0]?.title || 'Email Support',
      value: 'support@purrify.ca',
      description: contactPage?.contactMethods?.[0]?.description || 'Get detailed help with your questions',
      responseTime: contactPage?.contactMethods?.[0]?.responseTime || 'Response within 24 hours',
      action: 'mailto:support@purrify.ca',
    },
    {
      iconName: 'Phone',
      title: contactPage?.contactMethods?.[1]?.title || 'Phone Support',
      value: CONTACT_INFO.phone,
      description: PHONE_NUMBER.tagline,
      responseTime: contactPage?.contactMethods?.[1]?.responseTime || 'Mon-Fri 9AM-5PM EST',
      action: CONTACT_INFO.phoneHref,
      taglineNote: PHONE_NUMBER.description,
    },
    {
      iconName: 'MessageCircle',
      title: contactPage?.contactMethods?.[2]?.title || 'Live Chat',
      value: 'Chat with us',
      description: contactPage?.contactMethods?.[2]?.description || 'Chat with us in real-time',
      responseTime: contactPage?.contactMethods?.[2]?.responseTime || 'Available Now',
  ...
  const faqSchema = {
    ...
    '@type': 'FAQPage',
    mainEntity: contactPage?.faqs?.map((faq) => ({
      '@type': 'Question',
      name: faq.question,
      acceptedAnswer: {
        '@type': 'Answer',
            ...
            <div className="max-w-3xl mx-auto text-center">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 mb-8 rounded-full bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border border-purple-100 dark:border-purple-800/50 shadow-sm animate-fade-in">
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-gray-600 dark:text-gray-400">
                  {contactPage?.contactMethods?.[2]?.responseTime || 'Quick Response Time'}
                </span>
              </div>

              <h1 className="text-5xl md:text-7xl font-black mb-6 tracking-tight text-gray-900 dark:text-white leading-[1.1]">
                {contactPage?.title || 'Contact Us'} <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600 dark:from-purple-400 dark:to-pink-400">Support</span>
              </h1>

              <p className="text-lg md:text-xl text-gray-600 dark:text-gray-400 mb-10 leading-relaxed max-w-2xl mx-auto">
                {contactPage?.subtitle || 'Our friendly customer support team is ready to assist you with expert advice and solutions.'}
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
              {contactMethods.map((method, index) => (
                <ContactMethodCard
                  ...
                  contactNowText={contactPage?.form?.contactNow || 'Contact Now'}
                />
              ))}
            </div>
          </Container>
          ...
          <Container>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
              ...
              <div className="bg-white dark:bg-gray-800 p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-gray-100 dark:border-gray-700/50">
                <ContactForm translations={contactPage?.form || {}} locale={locale} />
              </div>
            </div>
          </Container>
        </section>
        ...
        <section className="py-24">
          <Container>
            <div className="text-center mb-16">
              <h2 className="text-3xl md:text-5xl font-black text-gray-900 dark:text-white mb-6">
                Frequently Asked <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-600 dark:from-purple-400 dark:to-pink-400">Questions</span>
              </h2>
              <p className="text-gray-500 dark:text-gray-400">{contactPage?.quickAnswersCommon || 'Quick answers to common questions'}</p>
            </div>
            ...
            <div className="text-center mt-12">
              <Link href="/#faq">
                ...
                >
                  {contactPage?.viewCompleteFAQ || 'View Complete FAQ'} →
                </Button>
              </Link>
            </div>
          </Container>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-24">
              <div className="p-8 md:p-12 rounded-[2.5rem] bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-800">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <h3 className="text-2xl font-black text-gray-900 dark:text-white">{contactPage?.businessHours?.title || 'Business Hours'}</h3>
                </div>
                <div className="space-y-4">
                  {[
                    { label: 'Weekdays', val: contactPage?.businessHours?.weekdays || 'Monday - Friday: 9:00 AM - 5:00 PM EST' },
                    { label: 'Saturday', val: contactPage?.businessHours?.saturday || 'Saturday: 10:00 AM - 2:00 PM EST' },
                    { label: 'Sunday', val: contactPage?.businessHours?.sunday || 'Sunday: Closed' },
                  ].map((item, i) => (
              ...
              <div className="p-8 md:p-12 rounded-[2.5rem] bg-gray-50 dark:bg-gray-900/50 border border-gray-100 dark:border-gray-800">
                <div className="flex items-center gap-4 mb-8">
                  ...
                  <h3 className="text-2xl font-black text-gray-900 dark:text-white">{contactPage?.location?.title || 'Our Location'}</h3>
                </div>
                ...
                <p className="text-sm font-medium text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20 px-4 py-2 rounded-full inline-block">
                  {contactPage?.location?.shippingNote || 'We ship across Canada and offer local pickup in the Montreal area.'}
                </p>
              </div>
            </div>
          </Container>
