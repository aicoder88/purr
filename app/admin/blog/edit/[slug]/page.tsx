...
export default function EditPostPage({ params }: EditPostPageProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const data = await fetchPostData(resolvedSlug);
  ...
  const performAutoSave = useCallback(async () => {
    if (!initialPost || !title.trim() || !content.trim()) {
      return;
    }
  ...
  const {
    ...
    delay: 30000,
    localStorageKey: `blog-draft-edit-${slug}`
  });
  ...
  }, [title, content, excerpt, selectedCategories, selectedTags, featuredImage, status, scheduledDate, triggerAutoSave, saveToLocalStorage, initialPost]);
