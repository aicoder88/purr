  ...
  return (
    <div className="space-y-16">
      ...
      <section className="text-center">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold bg-gradient-to-r from-emerald-500/15 to-blue-500/15 text-emerald-700 text-emerald-300 border border-emerald-500/30 border-emerald-500/20 mb-6">
          <Shield className="h-4 w-4" />
          {t.hero.badge}
        </div>
        <h2 className="font-heading text-5xl md:text-6xl font-black text-gray-900 text-white mb-6">
          <span className="block">{t.hero.titleLine1}</span>
          <span className="block bg-gradient-to-r from-emerald-500 to-blue-500 bg-clip-text text-transparent">
            {t.hero.titleLine2}
          </span>
        </h2>
        <p className="text-xl md:text-2xl text-gray-600 text-gray-300 mb-8">
          {t.hero.description}{' '}
          <strong className="text-emerald-600 text-emerald-400">
            {t.hero.highlight}
          </strong>
        </p>
        {/* Stats */}
        <div className="grid grid-cols-3 gap-4 max-w-lg mx-auto mb-8">
          <div className="text-center p-4 bg-white bg-gray-800 rounded-xl shadow-lg">
            <div className="text-3xl font-black text-emerald-600 text-emerald-400">100%</div>
            <div className="text-sm font-semibold text-gray-600 text-gray-300">
              {t.hero.stats.natural}
            </div>
          </div>
          <div className="text-center p-4 bg-white bg-gray-800 rounded-xl shadow-lg">
            <div className="text-3xl font-black text-blue-600 text-blue-400">0</div>
            <div className="text-sm font-semibold text-gray-600 text-gray-300">
              {t.hero.stats.chemicals}
            </div>
          </div>
          <div className="text-center p-4 bg-white bg-gray-800 rounded-xl shadow-lg">
            <div className="text-3xl font-black text-emerald-600 text-emerald-400">7+</div>
            <div className="text-sm font-semibold text-gray-600 text-gray-300">
              {t.hero.stats.days}
            </div>
          </div>
      ...
      {/* Benefits */}
      <section className="bg-gradient-to-br from-gray-50 via-white to-gray-50 from-gray-900 via-gray-800 to-gray-900 rounded-2xl p-8">
        <div className="text-center mb-12">
          <div className="inline-flex items-center px-4 py-2 rounded-full bg-gradient-to-r from-emerald-500/10 to-blue-500/10 text-emerald-700 text-emerald-300 font-semibold text-sm mb-6">
            {t.benefits.badge}
          </div>
          <h3 className="font-heading text-4xl md:text-5xl font-black text-gray-900 text-white mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {t.benefits.items.map((benefit, index) => (
            <div
              key={index}
              className="bg-white bg-gray-800 rounded-3xl p-8 shadow-xl border border-gray-200/50 border-gray-700/50"
            >
              <div className="flex items-start">
                <div
                  className={`w-16 h-16 bg-gradient-to-r ${benefit.color} rounded-2xl flex items-center justify-center mr-6 flex-shrink-0`}
                >
                  <FlaskConical className="h-8 w-8 text-white text-white" />
                </div>
                <div>
                  <h4 className="font-heading font-black text-xl text-gray-900 text-white mb-3">
                    {benefit.title}
                  </h4>
                  <p className="text-gray-700 text-gray-200">{benefit.description}</p>
                </div>
              </div>
            </div>
          ))}
      ...
      <section id="vet-contact">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {t.partnership.tiers.map((tier, index) => (
            <div
              key={index}
              className={`relative bg-white bg-gray-800 rounded-3xl p-8 shadow-xl border-2 ${tier.popular
                  ? 'border-emerald-500 border-emerald-400 transform md:-translate-y-4'
            ...
            >
              ...
              <div className="text-center mb-8">
                <h4 className="font-heading text-2xl font-black text-gray-900 text-white mb-2">
                  {tier.name}
                </h4>
                <p className="text-gray-600 text-gray-300">{tier.description}</p>
              </div>
              <ul className="space-y-4 mb-8">
                {tier.features.map((feature, featureIndex) => (
                  <li key={featureIndex} className="flex items-start">
                    ...
                    <span className="text-gray-700 text-gray-200">{feature}</span>
                  </li>
                ))}
              </ul>
              <Button
                className={`w-full py-4 font-bold rounded-xl ${tier.popular
                    ...
                    : 'bg-gray-100 bg-gray-700 text-gray-900 text-white'
                  }`}
              >
