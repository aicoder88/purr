...
function BalanceCard({
    ...
    return (
        ...
        >
            <div className="flex items-center justify-between mb-2">
                ...
                <div
                    className={`p-2 rounded-lg ${highlight
                            ? 'bg-purple-100 bg-purple-900/50'
            ...
            {description && (
                <p className="text-sm text-gray-500 text-gray-400 mt-1">{description}</p>
            )}
        </div>
    );
}
    ...
    const getStatusBadge = (status: string) => {
        const styles = {
            PENDING: {
                ...
                text: 'text-yellow-700 text-yellow-300',
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <BalanceCard
                                ...
                                icon={<Clock className="w-5 h-5 text-gray-600 text-gray-400" />}
                                description="30-day hold period"
                            />
                            <BalanceCard
                                title="Total Earnings"
                                value={data.balance.totalEarnings}
                                icon={<DollarSign className="w-5 h-5 text-gray-600 text-gray-400" />}
                                description="Lifetime earnings"
                            />
                            <BalanceCard
                                title="Total Paid Out"
                                value={data.balance.totalPaidOut}
                                icon={<CheckCircle className="w-5 h-5 text-gray-600 text-gray-400" />}
                            ...
                            {data.hasPendingRequest ? (
                                <div className="bg-yellow-50 bg-yellow-900/20 border border-yellow-200 border-yellow-800 rounded-lg p-4">
                                    <div className="flex items-start space-x-3">
                                        <Clock className="w-5 h-5 text-yellow-600 text-yellow-400 flex-shrink-0 mt-0.5" />
                                        <div>
                                ...
                                <div className="bg-gray-50 bg-gray-700/50 rounded-lg p-4">
                                    <div className="flex items-start space-x-3">
                                        <Settings className="w-5 h-5 text-gray-500 text-gray-400 flex-shrink-0 mt-0.5" />
                                        <div>
                                            <p className="font-medium text-gray-700 text-gray-200">
                                            ...
                                            <Link
                                                ...
                                                className="inline-flex items-center text-sm text-purple-600 text-purple-400 hover:text-purple-700 hover:text-purple-300 mt-2"
                                            >
                                                Go to Settings
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                                    <div>
                                        <p className="text-gray-600 text-gray-400">
                                            You have <span className="font-semibold text-gray-900 text-gray-100">
                                    ...
                                    <button
                                        ...
                                        className="mt-4 sm:mt-0 px-6 py-2 bg-purple-600 bg-purple-500 hover:bg-purple-700 hover:bg-purple-600 text-white text-gray-100 font-medium rounded-lg transition-colors"
                                    >
                                        Request Payout
                                    </button>
                                </div>
                        ...
                        <div className="bg-white bg-gray-800 rounded-xl border border-gray-200 border-gray-700 overflow-hidden">
                            ...
                            {data.payouts.length === 0 ? (
                                <div className="px-6 py-12 text-center">
                                    <Wallet className="w-12 h-12 text-gray-300 text-gray-600 mx-auto mb-4" />
                                        ...
                                        <tbody className="divide-y divide-gray-200 divide-gray-700">
                                            {data.payouts.map((payout) => (
                                                <tr key={payout.id}>
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 text-gray-100 text-right">
