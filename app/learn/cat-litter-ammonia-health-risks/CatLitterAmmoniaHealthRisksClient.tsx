  ...
  return (
    <div className="bg-cream-50 bg-gray-900 min-h-screen">
      <Container>
        ...
        <nav aria-label="Breadcrumb" className="pt-6 pb-4 flex items-center text-sm text-gray-500 text-gray-400">
          <Link href="/" className="hover:text-forest-600 hover:text-forest-400 transition-colors">
            <Home className="w-4 h-4" />
            <span className="sr-only">Home</span>
          </Link>
          <ChevronRight className="w-4 h-4 mx-2" />
          <Link href="/learn/" className="hover:text-forest-600 hover:text-forest-400 transition-colors">
            Learn
          </Link>
          <ChevronRight className="w-4 h-4 mx-2" />
          <span className="text-gray-900 text-gray-100 font-medium">Ammonia Health Risks</span>
        </nav>
        ...
        <article className="max-w-4xl mx-auto py-8 md:py-12 px-4">
          ...
          <header className="mb-12 text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-100 text-red-800 bg-red-900/50 text-red-200 text-sm font-medium mb-6">
            ...
            <p className="text-xl text-gray-600 text-gray-300 max-w-2xl mx-auto leading-relaxed">
              Understanding the health risks of cat litter ammonia and how to keep your family safe.
            </p>
          </header>

          {/* Quick Answer Box */}
          <div className="bg-white bg-gray-800 rounded-2xl p-8 shadow-xl border border-gray-200 border-gray-700 mb-12">
            <h2 className="text-2xl font-bold mb-4 text-gray-900 text-gray-100 flex items-center gap-3">
          ...
          <div className="prose prose-lg prose-forest max-w-none prose-invert">

            {/* What is Ammonia */}
            <h2 className="text-3xl font-bold mb-6 text-gray-900 text-gray-100">Where Does Litter Box Ammonia Come From?</h2>
            ...
            <div className="bg-gray-50 bg-gray-800 rounded-xl p-6 my-8">
              <div className="grid md:grid-cols-2 gap-8 items-center">
                <div>
                  <h3 className="text-xl font-bold text-gray-900 text-gray-100 mb-3">The Chemistry</h3>
                  <p className="text-gray-700 text-gray-300 mb-4">
                  ...
                  <div className="bg-white bg-gray-700 rounded-lg p-4 text-center font-mono text-gray-700 text-gray-300">
                    Urea + H₂O → 2NH₃ (ammonia) + CO₂
                  </div>
                  <p className="text-gray-600 text-gray-400 text-sm mt-4">
            ...
            {/* Exposure Levels */}
            <h2 className="text-3xl font-bold mb-6 text-gray-900 text-gray-100">Ammonia Exposure Levels</h2>
            ...
            <div className="overflow-hidden rounded-2xl border border-gray-200 border-gray-700 shadow-lg mb-8">
              <div className="grid grid-cols-4 bg-gray-50 bg-gray-800 p-4 border-b border-gray-200 border-gray-700 font-bold text-gray-500 text-gray-400 text-sm uppercase tracking-wider">
                <div>Level</div>
                <div className="col-span-2">Description</div>
                <div className="text-center">Risk</div>
              </div>
              {exposureLevels.map((item, i) => (
                <div key={i} className={`grid grid-cols-4 p-4 items-center ${i % 2 === 0 ? 'bg-white bg-gray-900' : 'bg-gray-50/50 bg-gray-800/50'}`}>
                  <div className="font-semibold text-gray-900 text-gray-100">{item.level}</div>
                  <div className="col-span-2 text-gray-700 text-gray-300 text-sm">{item.description}</div>
                  <div className="text-center">
                    <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${item.color === 'green' ? 'bg-green-100 text-green-800 bg-green-900/50 text-green-300' :
                        item.color === 'yellow' ? 'bg-yellow-100 text-yellow-800 bg-yellow-900/50 text-yellow-300' :
                          item.color === 'orange' ? 'bg-orange-100 text-orange-800 bg-orange-900/50 text-orange-300' :
                            'bg-red-100 text-red-800 bg-red-900/50 text-red-300'
                      }`}>
                      {item.risk}
                    </span>
                  </div>
            ...
            {/* Who is at Risk */}
            <h2 className="text-3xl font-bold mb-6 mt-12 text-gray-900 text-gray-100">Who is at Risk?</h2>
            ...
            <div className="grid md:grid-cols-3 gap-6 my-8">
              {riskGroups.map((group, index) => (
                <div key={index} className="bg-white bg-gray-800 rounded-xl p-6 shadow-lg border border-gray-200 border-gray-700">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="p-2 bg-forest-100 bg-forest-900/50 rounded-lg">
                      <group.icon className="w-5 h-5 text-forest-600 text-forest-400" />
                    </div>
                    <h3 className="font-bold text-lg text-gray-900 text-gray-100">{group.title}</h3>
                  </div>
                  <ul className="space-y-2 mb-4">
                    {group.risks.map((risk, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-gray-700 text-gray-300">
                        <X className="w-4 h-4 text-red-500 text-red-400 mt-0.5 flex-shrink-0" />
                        {risk}
                      </li>
                    ))}
                  </ul>
                  <p className="text-xs text-gray-500 text-gray-400 italic">{group.note}</p>
                </div>
              ))}
            </div>

            {/* How to Reduce Ammonia */}
            <h2 className="text-3xl font-bold mb-6 mt-12 text-gray-900 text-gray-100">How to Reduce Ammonia Exposure</h2>

            <div className="grid md:grid-cols-2 gap-6 my-8">
              {preventionTips.map((tip, index) => (
                <div key={index} className="bg-gradient-to-br from-forest-50 to-white from-gray-800 to-gray-800 rounded-xl p-6 border border-forest-100 border-forest-900/30">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="p-2 bg-forest-100 bg-forest-900/50 rounded-lg">
                      <tip.icon className="w-5 h-5 text-forest-600 text-forest-400" />
                    </div>
                    <h3 className="font-bold text-lg text-gray-900 text-gray-100">{tip.title}</h3>
                  </div>
                  <p className="text-gray-700 text-gray-300 text-sm">{tip.description}</p>
                </div>
              ))}
            </div>

            {/* Why Activated Carbon Works */}
            <div className="bg-forest-900 bg-forest-950 text-white text-gray-100 rounded-2xl p-8 my-12">
              <div className="grid md:grid-cols-2 gap-8 items-center mb-8">
                <div>
                  <h3 className="text-2xl font-bold mb-4">Why Activated Carbon is the Best Solution</h3>
                  <p className="text-forest-100 text-forest-200">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div className="bg-white/10 bg-white/10 rounded-lg p-4">
                  <Check className="w-5 h-5 text-green-400 text-green-300 mb-2" />
              ...
              <Link
                href="/products/"
                className="inline-flex items-center gap-2 bg-white bg-gray-100 text-forest-900 text-forest-950 font-medium px-6 py-3 rounded-lg mt-6 hover:bg-forest-50 hover:bg-gray-200 transition-colors"
              >
            ...
            {/* When to Seek Help */}
            <h2 className="text-3xl font-bold mb-6 text-gray-900 text-gray-100">When to Be Concerned</h2>

            <div className="bg-red-50 bg-red-900/20 border border-red-200 border-red-800 rounded-xl p-6 my-8">
              <h3 className="text-xl font-bold text-red-800 text-red-200 mb-4 flex items-center gap-2">
                <AlertTriangle className="w-5 h-5" />
                Seek Medical Attention If:
              </h3>
              <ul className="space-y-2 text-red-700 text-red-300">
              ...
              <p className="text-red-600 text-red-400 text-sm mt-4">
                These symptoms are rare with normal household exposure but warrant prompt attention.
              </p>
            </div>

            {/* Summary */}
            <h2 className="text-3xl font-bold mb-6 text-gray-900 text-gray-100">The Bottom Line</h2>
            ...
            <ul className="space-y-2 text-gray-700 text-gray-300 mb-6">
              <li className="flex items-start gap-2">
            ...
            <div className="mt-12 p-6 bg-gray-50 bg-gray-800 rounded-xl border border-gray-200 border-gray-700">
              <h3 className="font-heading font-bold text-gray-900 text-gray-100 mb-4">Related Ammonia Guides</h3>
              <div className="space-y-3">
                <Link href="/blog/how-to-neutralize-ammonia-cat-litter" className="flex items-center gap-2 text-[#FF3131] text-[#FF5050] hover:underline font-medium">
                  → Stop That Ammonia Smell: Complete Solution Guide
                </Link>
                <Link href="/learn/ammonia-science/" className="flex items-center gap-2 text-[#FF3131] text-[#FF5050] hover:underline font-medium">
          ...
          <footer className="mt-16 pt-8 border-t border-gray-200 border-gray-700 flex flex-col md:flex-row justify-between items-center text-sm text-gray-500 text-gray-400">
            <p>Published January 2025</p>
            <Link href="/learn/" className="mt-4 md:mt-0 px-6 py-2 bg-gray-100 hover:bg-gray-200 bg-gray-800 hover:bg-gray-700 rounded-full transition-colors duration-200">
              ← Back to Learn
            </Link>
          </footer>
        ...
        {/* Related Articles */}
        <div className="py-12 border-t border-gray-100 border-gray-800">
