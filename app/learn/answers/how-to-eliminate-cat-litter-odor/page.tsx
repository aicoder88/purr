...
export const metadata: Metadata = {
  title: 'How to Eliminate Cat Litter Odor - Quick Action Guide | Purrify',
          ...
          <div className="border-t border-gray-200 pt-8">
            ...
            <div className="space-y-4">
              {[
                ...
                <div 
                  key={item.step} 
                  className={`flex items-start gap-4 p-4 rounded-lg ${item.highlight ? 'bg-[#F7A41D]/10 border border-[#F7A41D]/30' : 'bg-gray-50'}`}
                >
                  <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${item.highlight ? 'bg-[#F7A41D] text-white' : 'bg-gray-200 text-gray-700'}`}>
                    {item.step}
                  </div>
                  <div className="flex-grow">
                    <p className={`font-medium ${item.highlight ? 'text-gray-900' : 'text-gray-700'}`}>
                      {item.action}
