...
import NaturalCatLitterAdditivePageClient from '@/app/learn/solutions/natural-cat-litter-additive/NaturalCatLitterAdditivePageClient';
...
const heroImage = `${SITE_URL}${LEARN_PAGE_PREVIEW_IMAGES['/learn/solutions/natural-cat-litter-additive'].image}`;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const canonicalPath = locale === 'en'
        ? `${SITE_URL}/learn/solutions/natural-cat-litter-additive/`
        : `${SITE_URL}/${locale}/learn/solutions/natural-cat-litter-additive/`;
...
const faqSchema = {
    ...
    mainEntity: [
        ...
        },
        {
            ...
            acceptedAnswer: {
                '@type': 'Answer',
                text: 'Both are safe, but activated carbon is more effective. Baking soda only provides 1-2 days of odor control and can cause digestive upset if cats ingest large amounts. Activated carbon works 5-7 days and passes through safely even if ingested.',
            },
        },
        {
            '@type': 'Question',
...
export default async function NaturalCatLitterAdditivePage({ params }: PageProps) {
    const { locale } = await params;
