...
import SolutionsPageClient from '@/app/learn/solutions/SolutionsPageClient';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    const title = `Cat Litter Odor Solutions - Expert Guides | ${SITE_NAME}`;
    const description = 'Find the right solution for your cat litter odor problem. Expert guides for ammonia smell, apartment living, multiple cats, senior cats, and more.';
    const canonicalPath = locale === 'en' ? `${SITE_URL}/learn/solutions/` : `${SITE_URL}/${locale}/learn/solutions/`;
...
export default async function LocalizedSolutionsPage({ params }: PageProps) {
    const { locale } = await params;
    if (!isValidLocale(locale)) {
        notFound();
    }
