...
export function createCityPage(slug: string) {
  const CityPage = () => {
    ...
    const cityRecord = getCityBySlug(slug);

    if (!cityRecord) {
      throw new Error(`Missing city profile for ${slug}`);
    }
              ...
              <div className="grid gap-10 lg:grid-cols-2 items-start">
                ...
                <div className="order-1 lg:order-2">
                  <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-xl p-6">
                    ...
                    <div className="space-y-6">
                      <div>
                        <span className="text-sm uppercase tracking-wide text-orange-600 dark:text-orange-300 font-semibold">
                          Climate Watch
                        </span>
                        <ul className="mt-2 space-y-2 text-gray-700 dark:text-gray-200">
                          {climateInsights.map((insight) => (
                            <li key={insight} className="flex items-start">
                              <span className="text-orange-500 dark:text-orange-300 mr-2 mt-1">•</span>
                              <span>{insight}</span>
                            </li>
                          ))}
                        </ul>
                      ...
                      <div>
                        <span className="text-sm uppercase tracking-wide text-blue-600 dark:text-blue-300 font-semibold">
                          Odor Control Focus
                        </span>
                        <ul className="mt-2 space-y-2 text-gray-700 dark:text-gray-200">
                          {scentPainPoints.map((pain) => (
                            <li key={pain} className="flex items-start">
                      ...
                      <div>
                        <span className="text-sm uppercase tracking-wide text-green-600 dark:text-green-300 font-semibold">
                          Keyword Targets
                        </span>
                        <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                          Monitor Search Console for phrases like {profile.englishQueries.slice(0, 2).join(', ')} and `{`cat litter ${profile.name}`}` to capture local demand.
                        </p>
                      </div>
                    </div>
                  </div>
