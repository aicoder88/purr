...
const CITY_COORDINATES = {
  ...
  quebec: { lat: 46.8139, lng: -71.2080, zoom: 11 }
};
...
export const ClientLocationsMap: React.FC<ClientLocationsMapProps> = ({
  ...
  const [activeCity, setActiveCity] = React.useState<keyof typeof CITY_COORDINATES | null>(null);

  const title = headerTitle || t.maps?.findNearYou || 'Find Purrify Near You | Retail Store Locations';
  const description = headerDescription || t.maps?.discoverWhere || 'Discover where to buy Purrify across Canada.';
    ...
    <section className={`py-8 ${className}`}>
      ...
      <div className="container mx-auto px-4">
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm relative min-h-[500px]">
          {isLoading && (
            <div className={`absolute inset-0 z-10 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-lg m-4 animate-pulse`} style={{ height }}>
              <div className="text-center">
                <svg className="w-10 h-10 text-gray-400 dark:text-gray-300 animate-spin mx-auto mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                <p className="text-gray-500 dark:text-gray-300 font-medium">Loading Map...</p>
              </div>
            </div>
          )}
          ...
          <iframe
            ...
            width="100%"
            height={height}
            style={{ border: 0, opacity: isLoading ? 0 : 1, transition: 'opacity 0.5s ease-in-out' }}
            allowFullScreen
            loading="lazy"
          ...
          <div className="mt-4 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-2 text-xs text-gray-500 dark:text-gray-400">
              {(Object.keys(CITY_COORDINATES) as Array<keyof typeof CITY_COORDINATES>).map((cityKey) => (
                <button
                  key={cityKey}
                ...
                >
                  {t.maps?.cities[cityKey] || cityKey.charAt(0).toUpperCase() + cityKey.slice(1)}
                </button>
              ))}
            </div>
          </div>
