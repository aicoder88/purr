...
const PLATFORMS = [
  { name: 'Facebook', icon: Facebook, limit: 500, color: 'bg-blue-500 dark:bg-blue-600' },
  { name: 'LinkedIn', icon: Linkedin, limit: 1300, color: 'bg-blue-700 dark:bg-blue-800' },
  { name: 'Twitter/X', icon: Twitter, limit: 280, color: 'bg-gray-900 dark:bg-gray-700' },
  { name: 'Instagram', icon: Instagram, limit: 2200, color: 'bg-pink-500 dark:bg-pink-600' },
  { name: 'Blog', icon: FileText, limit: 5000, color: 'bg-teal-500 dark:bg-teal-600' }
];
...
export default function SocialPage() {
  ...
  return (
    <>
      <Head>
        <title>Social Media - Purrify Hub</title>
      </Head>
      <OpsLayout>
        {/* Header */}
        <div className="mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          {PLATFORMS.map((platform) => (
            <div
              key={platform.name}
              className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 text-center hover:shadow-md dark:hover:shadow-gray-900/30 transition-shadow"
            >
              <div className={`w-12 h-12 ${platform.color} rounded-lg mx-auto mb-3 flex items-center justify-center`}>
                <platform.icon className="w-6 h-6 text-white dark:text-gray-100" />
              </div>
              <h3 className="font-medium text-gray-900 dark:text-gray-50">{platform.name}</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400">{platform.limit} chars</p>
            </div>
          ))}
        </div>
        ...
        {/* Tips Section */}
        <div className="mt-8 bg-gradient-to-r from-teal-50 to-cyan-50 dark:from-teal-900/20 dark:to-cyan-900/20 rounded-xl border border-teal-200 dark:border-teal-800 p-6">
