...
function generateSitemap(): string {
  ...
  VALID_STATIC_PAGES.forEach((page) => {
    const url =
      page.loc === "/" ? CANONICAL_BASE : `${CANONICAL_BASE}${page.loc}`;
  ...
  frenchPages.forEach((page) => {
    const url =
      page === "/" ? `${CANONICAL_BASE}/fr` : `${CANONICAL_BASE}/fr${page}`;
    urls.push({
      loc: url,
      lastmod: today,
  ...
  console.log(
    `   - English pages: ${urls.filter((u) => !u.loc.includes("/fr") && !u.loc.includes("/zh")).length}`,
  );
  console.log(
    `   - French pages: ${urls.filter((u) => u.loc.includes("/fr")).length}`,
  );
  console.log(
    `   - Chinese pages: ${urls.filter((u) => u.loc.includes("/zh")).length}`,
  );
...
function main() {
  ...
  console.log(
    "   ✅ Removed all redirect URLs (province codes, /checkout, etc.)",
  );
